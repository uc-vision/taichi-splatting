...
@cache
def adam_kernel(betas=(0.9, 0.999), eps=1e-08, weight_decay=0.0, use_individual_lr=False):
  b1, b2 = betas
  ...
  def kernel(param: ti.types.ndarray(dtype=ti.f32, ndim=2), grad: ti.types.ndarray(dtype=ti.f32, ndim=2),
             ...
             lr_scales: ti.types.ndarray(dtype=ti.f32, ndim=1),

             lr: ti.f32):

    for i in indexes:
      idx = indexes[i]
      lr_idx = lr * lr_scales[idx] if ti.static(use_individual_lr) else lr
      ...
      for j in range(param.shape[1]):
        ...
        param[idx, j] -= lr_idx * avg / (ti.sqrt(avg_sq) + eps)
...
class SparseAdam(torch.optim.Optimizer):
  def __init__(self, params, lr=0.001, 
    ...
    defaults = dict(lr=lr, betas=betas, eps=eps, weight_decay=weight_decay, scaled=None)
  ...
  def step(self, visible_indexes: torch.Tensor):
    for group in self.param_groups:
      ...
      if group['mode'] == 'adam':
        ...
        use_individual_lr = group["scaled"] is not None
        kernel = adam_kernel(betas=group["betas"], eps=group["eps"], 
                            weight_decay=group["weight_decay"], use_individual_lr=use_individual_lr)
        
        if group["scaled"] is not None:
          param.grad /= group["scaled"].unsqueeze(1)
          kernel(param, grad, exp_avg, exp_avg_sq, visible_indexes, lr_scales=group["scaled"], lr=group["lr"])
